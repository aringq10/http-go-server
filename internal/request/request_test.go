...
func TestBodyParse(t *testing.T) {
    ...
    reader = &chunkReader{
        ...
        "Content-Length: 20\r\n" +
        "\r\n" +
        "partial content, which is bigger!!!",
        numBytesPerRead: 3,
    }
    r, err = RequestFromReader(reader)
    require.Error(t, err)
...
func TestHeadersParse(t *testing.T) {
    ...
    require.NoError(t, err)
    require.NotNil(t, r)
    assert.Equal(t, requestStateDone, r.state)
    assert.Equal(t, 0, len(r.Headers))

    // Test: Duplicate headers
    reader = &chunkReader{
        data:            "GET / HTTP/1.1\r\nSet-User: kakius bakius   \r\nSet-User: sikius-makius\r\n\r\n",
        numBytesPerRead: 10,
    }
    r, err = RequestFromReader(reader)
    require.NoError(t, err)
    require.NotNil(t, r)
    assert.Equal(t, "kakius bakius, sikius-makius", r.Headers["set-user"])
